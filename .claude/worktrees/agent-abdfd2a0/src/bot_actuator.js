        ...
        try {
            ...
            // ── goto (waypoints, internal, journeyMap, /locate, no distance cap) ─
                ...
                if (destY !== undefined) {
                    ...
                    // Computes the next waypoint from current position and returns it, or null
                    // if the bot is already at the destination.
                    const _computeNextStreamWp = (cx, cz) => {
                        // Try path cache first
                        if (_cacheWps) {
                            const rdx = destX - cx, rdz = destZ - cz;
                            // Advance past already-visited cache entries
                            ...
                            // Cache exhausted — switch to fresh A*
                            _cacheWps = null;
                        }
                        const rdx = destX - cx, rdz = destZ - cz;
                    ...
                    // HANDOFF_DIST: switch to the next waypoint this many blocks before reaching
                    // the current one, so path computation for the next segment starts while
                    // the bot is still moving — eliminating any stop between segments.
                    const HANDOFF_DIST = 14;
                    // STALL_MS: time without meaningful movement before triggering stuck recovery.
                    const STALL_MS = 4000;
                    ...
                    while (!currentCancelToken.cancelled) {
                        ...
                        if (stalled > STALL_MS) {
                            // Bot hasn't moved in STALL_MS — trigger stuck recovery
                            _stLastTime = Date.now();
                            _stLastPos  = bot.entity.position.clone();
                            _stuckCount++;

                            if (_stuckCount <= 2) {
                                // Mild: jump to escape lip catches
                                bot.chat('[System] I am stuck. Trying jump escape...');
                                _cacheWps = null; // abandon stale cache path
                                if (movements.canDig === false) {
                                    movements.canDig = true;
                                    bot.pathfinder.setMovements(movements);
                                }
                                ...
                                } catch (_e) { /* try other side on next attempt */ }
                                _sw = _computeNextStreamWp(bot.entity.position.x, bot.entity.position.z);
                                if (!_sw) break;
                                bot.pathfinder.setGoal(new goals.GoalXZ(_sw.x, _sw.z), true);

                            } else {
                                // Severe: force-walk (only if safe ahead)
                                const af = Math.atan2(rdz, rdx);
                                if (isSafeForward(af)) {
                                    bot.pathfinder.setGoal(null);
                                ...
                                if (_stuckCount >= 7) {
                                    console.log('[Actuator] Stuck recovery exhausted after 7 attempts. Aborting goto.');
                                    break;
                                }
                                _sw = _computeNextStreamWp(bot.entity.position.x, bot.entity.position.z);
                                if (!_sw) break;
                        ...
                        // ── Lookahead handoff ────────────────────────────────────────────────
                        // When within HANDOFF_DIST of the current streaming waypoint, switch to
                        // the next one immediately. This starts A* for the next segment while the
                        // bot is still moving — zero inter-segment stop time.
                        if (_sw) {
                            ...
                            if (d2sw < HANDOFF_DIST) {
                                ...
                                _sw = nextSw;
                                // Switch goal immediately while still moving — no pause
                                bot.pathfinder.setGoal(new goals.GoalXZ(_sw.x, _sw.z), true);
                            }
                        }
                    }

                    // Stop streaming goal and restore dig setting
                    try { bot.pathfinder.setGoal(null); bot.clearControlStates(); } catch (_e) {}
                    movements.canDig = savedCanDig;
                    bot.pathfinder.setMovements(movements);

                    // Issue 2: save successful path to cache
